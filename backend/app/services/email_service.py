...
def send_security_news_digest(
    user_email: str,
    recipients: "str | list[str]",
    items: list,
    date_str: str,
) -> bool:
    """Send the daily cybersecurity news digest.

    No URLs are included — each story shows headline, plain-text summary,
    publish date, and source name only.
    """
    if not items:
    ...
    intro = (
        ...
        "No external links are included — search the headline for full details.</p>"
    )
    body_html = ""
    for item in items:
        title = html.escape(item.get("title") or "")
        summary = _nl2br(item.get("summary") or "")
        date = html.escape(item.get("date") or "")
        source = html.escape(item.get("source") or "")
        rows: list[tuple[str, str]] = [("Headline", f"<strong>{title}</strong>")]
        if summary:
            rows.append(("Summary", summary))
        if date:
            rows.append(("Published", date))
        rows.append(("Source", f'<em style="color:#475569;">{source}</em>'))
        body_html += _detail_rows(rows)
    source_list = "The Hacker News, Bleeping Computer, CISA Known Exploited Vulnerabilities"
    body_html += _note(
        f"Digest generated by Centriq AI on {html.escape(date_str)}. "
        f"Sources: {source_list}. No external links are included in this email."
    )
    html_body = _email_shell(
        f"Cybersecurity Digest — {html.escape(date_str)}",
        intro,
        body_html,
        preheader=f"Today's cybersecurity news — {len(items)} stories",
    )
    return _send_html(user_email, recipients, subject, html_body)
