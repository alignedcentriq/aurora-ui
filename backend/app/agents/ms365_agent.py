...
def ms365_assistant(state: MS365State):
    ...
    default_prompt = (
        ...
        f"- Send email → send_email_graph. Draft the email immediately from context and call the tool — "
        f"do NOT ask the user to confirm body/subject first. The confirmation card handles review. "
        f"If [MANAGER INFO] is in context, use that email directly — do NOT call list_org_users.\n"
