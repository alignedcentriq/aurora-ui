...
class HRService:
    ...
    @staticmethod
    def apply_leave(email, start_date, end_date, leave_type):
        emp = HRService.get_employee_by_email(email)
        if not emp: return "Employee not found."
        ...
        new_leave = {
            ...
            "end_date": end_date,
            "status": "Pending",
            "reason": "Applied via AI Assistant"
        }
    ...
    def search_policies(query):
        ...
        relevant = [p for p in policies if query.lower() in p["content"].lower() or query.lower() in p["title"].lower()]
        if not relevant:
            return "No specific policy found for your query. Please contact HR for more details."
