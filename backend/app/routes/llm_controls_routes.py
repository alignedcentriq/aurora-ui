"""IT-only endpoints for the runtime LLM controls (kill switch, load throttle,
per-tier model params, per-domain disable). Gated by ``require_it``."""
from fastapi import APIRouter, Body, Depends, HTTPException

from app.auth import CurrentUser, require_it
...
@router.get("")
async def get_llm_controls(_: CurrentUser = Depends(require_it)):
...
@router.post("/reset")
async def reset_llm_controls(user: CurrentUser = Depends(require_it)):
