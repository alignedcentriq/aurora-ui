...
type PortalTab = "leaves" | "welcome-logs" | "welcome-config";
...
export function HRPortal() {
  const { user } = useAuth();
  const [tab, setTab] = useState<PortalTab>("leaves");
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden">
        {tab === "leaves" && <LeavesTab authHeaders={authHeaders} />}
...
function LeavesTab({ authHeaders }: { authHeaders: Record<string, string> }) {
