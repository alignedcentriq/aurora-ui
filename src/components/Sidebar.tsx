...
export function Sidebar() {
  ...
  const navItems = [
    ...
    { to: "/config", icon: Database, label: "Prompt Config", show: ["Admin", "HR", "IT", "PMO"].includes(user.role) },
    { to: "/team", icon: Users, label: "Team Management", show: user.role === "Functional Manager" },
      ...
      <div className="mx-4 mt-4 mb-2">
        <div className="flex items-center gap-2.5 rounded-lg bg-white/[0.04] px-3 py-2">
          ...
          <div className="flex flex-col">
            <span className="text-[11px] font-semibold uppercase tracking-widest text-[var(--sidebar-foreground)]/40">Role</span>
            <span className="text-xs font-medium text-[var(--sidebar-foreground)]/80">{currentRoleMeta.label}</span>
          </div>
        </div>
      </div>
