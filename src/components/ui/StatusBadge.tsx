...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  if (cleanStatus === "sent") {
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1 text-xs font-medium text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-full px-2 py-0.5",
          className,
        )}
      >
  ...
  if (cleanStatus.startsWith("failed:no_recipients")) {
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1 text-xs font-medium text-amber-600 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5",
          className,
        )}
      >
  ...
  if (cleanStatus.startsWith("failed")) {
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1 text-xs font-medium text-red-600 bg-red-50 border border-red-200 rounded-full px-2 py-0.5",
          className,
        )}
      >
  ...
  // General badges
  const badgeClass =
    DEFAULT_STATUS_BADGE[cleanStatus] ??
    "bg-zinc-500/10 text-zinc-500 dark:text-zinc-400 border border-zinc-500/20";

  // Format the label
  const label =
    FRIENDLY_STATUS_LABELS[cleanStatus] ??
    status.replace(/[_-]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());

  return (
    <span
      className={cn(
        "rounded-full px-2.5 py-1 text-[11px] font-semibold transition-all inline-block whitespace-nowrap",
        badgeClass,
        className,
      )}
    >
