...
export function TravelExpenseForm({ userEmail, onSubmitted }: Props) {
  ...
  const [overLimit, setOverLimit] = useState<{ limit: number; currency: string; amount: number } | null>(null);
  ...
  const handleTripChange = (ref_id: string) => {
    ...
    setForm(f => ({ ...f, travel_ref_id: ref_id, currency: req?.expense_limit_currency || "INR" }));
    setOverLimit(null);
  };
        ...
        {loadingReqs ? (
          ...
          <>
            <div>
              ...
              <select value={form.travel_ref_id} onChange={e => handleTripChange(e.target.value)} className={inputCls} required>
                <option value="">— Select a trip —</option>
                {requests.map(r => (
                  <option key={r.ref_id} value={r.ref_id}>
                    {r.ref_id} — {r.from_location} → {r.to_destination} ({r.travel_date})
                    {r.expense_limit ? ` · Limit: ${r.expense_limit_currency || "INR"} ${r.expense_limit.toLocaleString()}` : ""}
                  </option>
                ))}
              </select>
              {selectedReq?.expense_limit && (
                <p className="text-[11px] text-[#64748b] dark:text-white/40 mt-1">
                  Approved expense limit: <strong>{limitCurrency} {selectedReq.expense_limit.toLocaleString()}</strong>
                </p>
              )}
            </div>

            <div>
              <label className={labelCls}>Total Amount <span className="text-rose-400">*</span></label>
              <div className="flex gap-2">
                <select value={form.currency} onChange={e => { set("currency", e.target.value); setOverLimit(null); }}
              ...
              {isOverLimit && (
                <p className="text-[11px] text-amber-600 mt-1 font-medium">
                  ⚠ This exceeds the approved limit by {limitCurrency} {(parseFloat(form.amount) - selectedReq!.expense_limit!).toLocaleString()}. A reason will be required.
                </p>
              )}
            </div>
