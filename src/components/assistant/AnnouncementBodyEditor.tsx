...
export interface ImageAction { type: ActionType; value: string; label: string }
...
export function AnnouncementBodyEditor({
  ...
  imageAction, onImageActionChange,
  authHeaders, title, category,
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/announcements/upload-image", { method: "POST", headers: uploadHeaders, body: form });
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    if (imageItem) { e.preventDefault(); const f = imageItem.getAsFile(); if (f) uploadImage(f); }
  };

  const handleDrop = (e: React.DragEvent<HTMLTextAreaElement>) => {
    setDragOver(false);
    const file = e.dataTransfer.files[0];
    if (file?.type.startsWith("image/")) { e.preventDefault(); uploadImage(file); }
  };
  ...
  const loadFormsAndApps = () => {
    const h = { "x-user-email": authHeaders["x-user-email"], "x-user-role": authHeaders["x-user-role"] };
    if (forms.length === 0) fetch("/api/forms/list", { headers: h }).then(r => r.ok ? r.json() : []).then(setForms).catch(() => {});
    if (apps.length === 0) fetch("/api/urls/list", { headers: h }).then(r => r.ok ? r.json() : []).then(setApps).catch(() => {});
  };

  const applyAction = () => {
    if (!actionValue.trim()) { toast.error("Enter a destination"); return; }
    onImageActionChange({ type: actionType, value: actionValue.trim(), label: actionLabel.trim() || actionValue.trim() });
    setShowLinkPicker(false);
  };

  const removeImage = () => {
    onImageUrlChange(null); onImageActionChange(null); setShowLinkPicker(false);
  };
        ...
        <>
          ...
          {imageAction ? (
            <div className="flex items-center gap-1.5 rounded-lg border border-border bg-muted/40 px-2.5 py-1.5">
              ...
              <button type="button" onClick={() => { onImageActionChange(null); setShowLinkPicker(true); }} className="ml-1 text-[10px] text-muted-foreground hover:text-foreground">Edit</button>
              <button type="button" onClick={() => onImageActionChange(null)} className="text-muted-foreground hover:text-rose-500 transition-colors"><X className="h-3 w-3" /></button>
            </div>
          ) : (
            <button
              type="button"
              onClick={() => { setShowLinkPicker((v) => !v); loadFormsAndApps(); }}
                ...
                <div className="flex gap-1">
                  {(["url", "form", "app"] as ActionType[]).map((t) => (
                    <button
                      key={t}
                      type="button"
                      onClick={() => { setActionType(t); setActionValue(""); setActionLabel(""); if (t !== "url") loadFormsAndApps(); }}
                        ...
                        <div className="max-h-36 overflow-y-auto space-y-1">
                          {apps.map((a) => (
                            ...
                            >
                              ...
                              {a.purpose && <span className="ml-1.5 text-[10px] text-muted-foreground">— {a.purpose.slice(0, 40)}</span>}
                            </button>
                          ))}
                        </div>
                      )}
